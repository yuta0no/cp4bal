...
def make_key_collatable(key: str) -> str:
    ...
    replaces = {"batch": "b#atch", "index": "i#ndex", "face": "f#ace"}
...
def make_mapping_collatable(d):
    ...
    for k, v in d.items():
        ...
        if k != old_k:
            logger.info(f'Making mapping collatable: Sanitized "{old_k}" to "{k}"')
        result[k] = v
    return result
...
def compress_labels(
    labels: Int[np.ndarray, " n"], label_to_idx: Dict[str, int]
) -> Tuple[Int[np.ndarray, " n"], Dict[str, int], Dict[int, int]]:
...
def remap_labels(
    labels: Int[np.ndarray, " n"], label_to_idx: Dict[str, int], remapping: Dict[int, int], undefined_label: int = -1
) -> Tuple[Int[np.ndarray, " n"], Dict[str, int]]:
...
def sample_from_mask(
    mask: Bool[np.ndarray, " n"], num_samples: int, rng: np.random.RandomState | None = None
) -> Bool[np.ndarray, " n"]:
